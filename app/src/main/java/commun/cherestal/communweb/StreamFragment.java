...
import commun.cherestal.communweb.adapter.AdvancedItemListAdapter;
...
import commun.cherestal.communweb.model.Item;
import commun.cherestal.communweb.util.Api;
import commun.cherestal.communweb.util.CustomRequest;
import commun.cherestal.communweb.util.ItemInterface;
...
public class StreamFragment extends Fragment implements Constants, SwipeRefreshLayout.OnRefreshListener, ItemInterface {
    ...
    //Lat lng of user based on instance id
    public static double idLongitude=0.0;
    public static double idLatitude=0.0;

    //distance between two user in meters
    ...
    public void onCreate(Bundle savedInstanceState) {
       ...
       //UserInfomation.getUserLocbyId(14);
    ...
    /** retrieves the distance value between two users
     * stored in shared preferences
     * distance in meters
     **/
    public static double isInVicinity(){
        SharedPreferences sharedPref = App.getInstance().getSharedPreferences(Context.LOCATION_SERVICE, 0);
        String mDistance = sharedPref.getString("distanceBetween", "0");

        Log.d(TAG, "This is the vicinity" + Double.parseDouble(mDistance)/1603.44);

        return Double.parseDouble(mDistance);
    }
    ...
    public void getItems() {
        ...
        //Updates User profile/instance ID  in Firebase
        UserInfomation.updateProfileID();
        Log.d(TAG, "This is the UserUUID" + UserInfomation.getUUIDbyProfileId(App.getInstance().getId()));

        //update user last known location to instance/profile id
        UserInfomation.setLatLngById(App.getInstance().getId());
        Log.d(TAG, "This is the latlng" + idLongitude);
                            ...
                            if (!response.getBoolean("error")) {
                                ...
                                if (response.has("items")) {
                                    ...
                                    if (arrayLength > 0){
                                        ...
                                        Log.d(TAG, "Distance in Stream Fragment" + distbetween/1603.344);
                                        for (int i = 0; i < itemsArray.length(); i++) {

                                            JSONObject itemObj = (JSONObject) itemsArray.get(i);


                                            //get profile/instance id of user from post
                                            int uid = itemObj.getInt("fromUserId");

                                            if(uid!=App.getInstance().getId()){
                                                //if uid is different, calculate distance between the users
                                                UserInfomation.distanceBetweenUser(App.getInstance().getId(), uid);
                                            }
                                            Item item = new Item(itemObj);
                                            item.setAd(0);

                                           //if post is from this user, add it to stream
                                           if(uid==App.getInstance().getId()){
                                               itemsList.add(item);
                                           }
                                           else {
                                               //post is not from this, calculate distance between the two
                                               // convert return distance in miles
                                               //if less than, add post to stream
                                               if(isInVicinity()/1603.344<100) {
                                                   itemsList.add(item);
                                               }
                                           }
